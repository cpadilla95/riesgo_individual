...
def obtener_riesgo_social(df, probit_df):
    ...
    df["Probabilidad Muerte"] = df.apply(
        lambda x: prob_muerte(x["Nivel de Afectación"], rosa_vientos, probit_df), axis=1
    )

    df["Densidad poblacional Día"] = df.apply(
        lambda x: densidad(x["Personas Dia"], x["Área total"], x["OFF SITE/ONSITE"]),
        axis=1,
    )
    df["Densidad poblacional Noche"] = df.apply(
        lambda x: densidad(x["Personas Noche"], x["Área total"], x["OFF SITE/ONSITE"]),
        axis=1,
    )

    df["Fatalidad Dia"] = df.apply(
        lambda x: num_personas(
            x["Personas Dia"],
            x["Densidad poblacional Día"],
            x["Area intersectada (m2)"],
            x["OFF SITE/ONSITE"],
        ),
        axis=1,
    )
    df["Fatalidad Noche"] = df.apply(
        lambda x: num_personas(
            x["Personas Noche"],
            x["Densidad poblacional Noche"],
            x["Area intersectada (m2)"],
            x["OFF SITE/ONSITE"],
        ),
        axis=1,
    ...
    df["Probabilidad de Fatalidad Dia"] = (
        df["Frecuencia SF"] * df["Probabilidad Muerte"] * 0.6
    )
    df["Probabilidad de Fatalidad Noche"] = (
        df["Frecuencia SF"] * df["Probabilidad Muerte"] * 0.4
    )
    df["Probabilidad de Fatalidad Total"] = (
        df["Probabilidad de Fatalidad Dia"] + df["Probabilidad de Fatalidad Noche"]
    )

    output = BytesIO()
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        df.to_excel(writer, sheet_name="Cruces", index=False)
        graph_rs(writer, df, "Dia", "ONSITE")
        graph_rs(writer, df, "Noche", "ONSITE")
        graph_rs(writer, df, "Total", "ONSITE")
        graph_rs(writer, df, "Dia", "OFFSITE")
        graph_rs(writer, df, "Noche", "OFFSITE")
        graph_rs(writer, df, "Total", "OFFSITE")
    output.seek(0)
    return output
...
def graph_rs(writer, df, tipo, onsite):
    ...
    limit_y = (
        0.000001 if graph_df["Frecuencia Acumulada"].min() > 0.000001 else 0.00000001
    )
