...
def riesgo_AAD(file, sustancias_df):
    df = pd.read_excel(
        ...
        skiprows=7,
        usecols="C:D, G:J, M, P:V, AA:AH, AL:AS, AW:BD, BH:BI, BM:BN, BR:BV, BZ:CD",
    )
...
#######################################################################
def riesgo_social(file):
    df = pd.read_excel(file, sheet_name="Cruces")
    df.replace(r"^\-$", np.NaN, inplace=True, regex=True)
    return df
